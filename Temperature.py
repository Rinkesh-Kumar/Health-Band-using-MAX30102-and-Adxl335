...
class Temperature():
    ...
    def shutdown(self):
        ...
        Shutdown the device.
        """
        self.bus.write_i2c_block_data(self.address,REG_TEMP_CONFIG, [0x80])
    ...
    def reset(self):
        ...
        """
        self.bus.write_i2c_block_data(self.address, REG_TEMP_CONFIG, [0x40])
    ...
    def setup(self, led_mode=0x03):
        ...
        self.bus.write_i2c_block_data(self.address, REG_TEMP_CONFIG, [led_mode])
        # 0b 0010 0111
    ...
    def getTemperature(self):
        self.bus.write_i2c_block_data(self.address,MAX30105_FIFOWRITEPTR, [0])
        self.bus.write_i2c_block_data(self.address, MAX30105_FIFOOVERFLOW, [0])
        self.bus.write_i2c_block_data(self.address, MAX30105_FIFOREADPTR, [0])
        intg=None
        frac=None
        intg=self.i2c.read_byte_data(self.address,REG_TEMP_INTR)
        frac=self.i2c.read_byte_data(self.address,REG_TEMP_FRAC)
        self.reset()
        reg_data = self.bus.read_i2c_block_data(self.address, REG_INTR_STATUS_1, 1)
        self.setup()
